...
io.sockets.on('connection', function (socket) {
    ...
    function sendMessage() { //This function is to achieve device to cloud function
        if (!sendingMessage) {
            return;
        }
    ...
    /*This function is Cloud to Device function(to show messages from Device Explorer)*/
    function receiveMessageCallback(msg) {
        var message1 = msg.getData().toString('utf-8');

        client.complete(msg, () => {
            console.log('Receive message: ' + message1);
            socket.emit('messagefromiot', message1); //emit to client end via socketio
        });
    }
